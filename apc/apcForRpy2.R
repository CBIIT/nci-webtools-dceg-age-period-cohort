...
# imageDirectory <- "~/myproject/app/static/csv/"; #"static/img";
# csvDirectory <- "~/myproject/app/static/csv/"; #"static/csv";
...
createPanCanList <- function (inputList) {
  ...
  panCan <- list(fullname = getTitle(inputList),
...
getApcDataJSON <-function(urlEncodedString)
{
  ...
  if (key == "AgeDeviations") {
    ...
    jsonString = toJSON(apcdata$Coefficients, method="C");
