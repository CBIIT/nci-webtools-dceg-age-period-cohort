DrawSensSpecLR <- function(uniquekey, sensref, specref,sensitivity,specificity, names) {
  file<- paste("./img/",uniquekey,"SensSpecLR.png")
  ...
  legend(lty=LTY,col=LTY,legend=c(paste("LR-",round(LRMinus,digits=1)),paste("LR+",round(LRPlus,digits=1))),"bottomright",cex=1.15,text.font=2,bty="o",title="Likelihood Ratio (LR)") 
  dev.off()
}
