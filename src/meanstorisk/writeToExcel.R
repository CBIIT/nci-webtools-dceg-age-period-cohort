...
writeResultsToExcel <- function (risk, graphName) {
  ...
  sgsSheet              <- createSheet(outwb, sheetName = "Sensitivity Given Specificity");
  ...
  addDataFrame(x = as.data.frame.matrix(risk$`Sensitivity Given Specificity`), sheet = sgsSheet, row.names=TRUE, col.name=TRUE);
  addDataFrame(x = as.data.frame.matrix(risk$PPV), sheet = ppvSheet, row.names=TRUE, col.name=TRUE);
  addDataFrame(x = as.data.frame.matrix(risk$cNPV), sheet = cNpvSheet, row.names=TRUE, col.name=TRUE);
  addDataFrame(x = as.data.frame.matrix(risk$`PPV-cNPV`), sheet = ppvCnpvSheet, row.names=TRUE, col.name=TRUE);
  addDataFrame(x = as.data.frame.matrix(risk$`Program-Based`), sheet = programBasedSheet, row.names=TRUE, col.name=TRUE);
  addDataFrame(x = as.data.frame.matrix(risk$`PPV-Based`), sheet = ppvBasedSheet, row.names=TRUE, col.name=TRUE);
  addDataFrame(x = as.data.frame.matrix(risk$`Sensitivity-Based`), sheet = sensitivityBasedSheet, row.names=TRUE, col.name=TRUE);
  addDataFrame(x = as.data.frame.matrix(risk$`Dominated by Specificity for a Rare Disease`), sheet = dominatedSheet, row.names=TRUE, col.name=TRUE);
