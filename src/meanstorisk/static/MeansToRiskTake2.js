// global variable for time. Thihs uniquely identifies the graph file per session
var uniqueKey;
var newDelta;
...
function set_standard_inputs(mean_cases,mean_controls,stderr_cases,stderr_controls,N_cases,N_controls,
	...
	set_value("#specificity_5",specificity_5);
        if (((specificity_1 > 1) || (specificity_1 < 0))
        || ((specificity_2 > 1) || (specificity_2 < 0))
        || ((specificity_3 > 1) || (specificity_3 < 0))
        || ((specificity_4 > 1) || (specificity_4 < 0))
        || ((specificity_5 > 1) || (specificity_5 < 0)))
        alert("Specificity value must be between 0 and 1.")
...
function refreshGraph(drawgraph) {
   if (drawgraph == 1)
      {
         graph_file = "../static/"+uniqueKey+"rplot.png?";
      }
   else
      {
         graph_file = "../static/images/fail-message.jpg?";
      }

   d = new Date();
   $("#SpecGraph").attr("src", graph_file+d.getTime());
}
...
function get_data_stream() {
	...
	$.ajax({
		type: "GET",
		url: "http://"+hostname+":8080/jsonp/DeltaSpPpv",
...
function format_number(num) {
	...
	//if (num < 100000 && num > 0.00001) return intermediate.toString();
