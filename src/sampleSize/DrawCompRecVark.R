DrawCompRecVarkSensSpec <- function(k,sens,spec,prev,N) {
  ...
  Y <- seq(from=-0.5,to=3,by=0.00001)
  list <- 1:length(sens)
  vals <-  1:length(sens)
  ...
  legend(pch=c(LTY,rep(NA,length(LTY))),lty=c(LTY,rep(NA,length(LTY))),legend=c(sens,round(vals,2)),"bottomright",cex=1.15, lwd=3,col= LTY,text.font=2,bty="n",inset=c(-0.75,0),xpd = TRUE,title="Sensitivity    Optimal k",ncol=2)
}
...
DrawCompRecVarkSpecSens <- function(k,spec,sens,prev,N) {
  ...
  legend(pch=c(LTY,rep(NA,length(LTY))),lty=c(LTY,rep(NA,length(LTY))),legend=c(spec,round(vals,2)),"bottomright",cex=1.15, lwd=3,col= LTY,text.font=2,bty="n",inset=c(-0.75,0),xpd = TRUE,title="Specificity    Optimal k",ncol=2)
}
